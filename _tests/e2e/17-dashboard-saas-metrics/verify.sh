...
check "has data.json in dist" "[ -f ${APP}dist/data.json ] || [ -f ${APP}public/data.json ] || [ -f ${APP}data.json ]"
check "has JS bundle" "ls ${APP}dist/assets/*.js >/dev/null 2>&1"
check "has CSS bundle" "ls ${APP}dist/assets/*.css >/dev/null 2>&1"
check "dist self-contained (no https)" "! grep -q 'https://' ${APP}dist/index.html 2>/dev/null || true"
